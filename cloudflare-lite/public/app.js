...
function eventCard(event) {
  ...
  return `<article class="card event-card"><div class="actions"><span class="status ${String(event.status).toLowerCase()}">${statusText(event.status)}</span><span class="tag">${esc(event.type)}</span></div><h3>${esc(event.title)}</h3><p>${esc(event.description)}</p><div class="event-meta"><span>时间：${formatDate(event.start_time)}</span><span>发起人：${esc(event.creator_name || "管理员")}</span><span>名额：${Number(event.approved_count || 0)} / ${Number(event.max_participants)}</span>${event.requirements ? `<span>要求：${esc(event.requirements)}</span>` : ""}${event.voice_channel ? `<span>语音：${esc(event.voice_channel)}</span>` : ""}</div>${participants.length ? `<div class="participant-list"><strong>已加入玩家</strong>${participants.map((player) => `<div class="participant-card">${avatar(player)}<div><b>${esc(player.displayName)}</b><div class="tags">${player.mainRole ? `<span class="tag">${esc(player.mainRole)}</span>` : ""}${list(player.mainHeroes).slice(0, 3).map((hero) => `<span class="tag">${esc(hero)}</span>`).join("")}</div></div></div>`).join("")}</div>` : ""}${event.status === "OPEN" ? `<button class="button primary small ${allowed ? "join" : "login-to-join"}" data-id="${esc(event.id)}">${allowed ? "加入活动" : "注册或登录后加入"}</button>` : ""}</article>`;
}
